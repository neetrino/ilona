 ...
 */
export async function uploadDashboardBanner(
  file: File,
): Promise<{ bannerUrl: string; key: string; mimeType: string; fileSize: number }> {
  const formData = new FormData();
  formData.append('file', file);
  ...
  try {
    const response = await api.post<{
      success: boolean;
      data: { bannerUrl: string; key: string; mimeType: string; fileSize: number };
    }>('/settings/dashboard-banner', formData);
    ...
    throw new Error('Failed to upload dashboard banner: Invalid response from server');
  } catch (error: unknown) {
    if (error instanceof ApiError) {
      if (error.statusCode === 413) {
        throw new Error('File size too large. Please upload an image smaller than 5MB.');
      ...
      if (error.statusCode === 403) {
        throw new Error('You do not have permission to upload dashboard banners. Admin access required.');
      }
    }
    throw error;
  }
