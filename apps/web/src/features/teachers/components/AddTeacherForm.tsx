...
import { useAddTeacherForm } from './add-teacher-form/useAddTeacherForm';
import { AddTeacherFormFields } from './add-teacher-form/AddTeacherFormFields';
import type { AddTeacherFormProps } from './add-teacher-form/add-teacher-form.types';

export type { AddTeacherFormProps } from './add-teacher-form/add-teacher-form.types';

export function AddTeacherForm(props: AddTeacherFormProps) {
  const form = useAddTeacherForm(props);
    ...
    <DialogPrimitive.Root open={form.isDialogOpen} onOpenChange={(nextOpen) => !nextOpen && form.requestClose()}>
      <DialogPrimitive.Portal>
        ...
        >
          ...
          <DialogPrimitive.Title className="sr-only">{form.tForm('addTitle')}</DialogPrimitive.Title>
          <div className="shrink-0 bg-[#f8f9fb] px-4 pb-4 pt-3 min-[1367px]:px-6 min-[1367px]:pb-5 min-[1367px]:pt-6">
            <div className="flex items-start justify-between gap-4">
              <div className="min-w-0 flex-1">
                <h2 className="text-lg font-semibold text-[#3b3b40]">{form.tForm('addTitle')}</h2>
              </div>
