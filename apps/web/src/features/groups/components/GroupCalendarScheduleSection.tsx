...
export interface GroupCalendarScheduleSectionProps {
  ...
  onCalendarEnabledChange: (next: boolean) => void;
  dateFrom: string;
  dateTo: string;
...
export function GroupCalendarScheduleSection({
  ...
  const preview = useMemo(() => {
    if (!calendarEnabled || !dateFrom || !dateTo || schedule.length === 0 || slotError) {
      return [];
    }
    return previewLessonOccurrences(schedule, dateFrom, dateTo);
  }, [calendarEnabled, dateFrom, dateTo, schedule, slotError]);
    ...
    <div className="space-y-4 rounded-lg border border-slate-200 bg-slate-50/40 p-3">
      <div className="space-y-2">
        ...
        <p className="text-xs text-slate-500">
          Weekly time slots plus an optional date range generate lessons in the admin calendar. Manually deleted
          generated slots stay skipped until you confirm replacing the schedule.
        </p>
        <label className="flex cursor-pointer items-center gap-2 text-sm">
          <input
            type="checkbox"
            checked={calendarEnabled}
            onChange={(e) => onCalendarEnabledChange(e.target.checked)}
            disabled={disabled}
            className="size-4 rounded border-slate-300 text-primary focus:ring-primary"
          />
          <span>Generate lessons in calendar for a date range</span>
        </label>
      </div>
      ...
      {calendarEnabled && preview.length > 0 && !slotError && (
