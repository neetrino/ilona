...
import { StudentsUniqueTotalStat } from './StudentsUniqueTotalStat';

type StripCenter = Pick<Center, 'id' | 'name'> & { colorHex?: string | null };

interface StudentsCentersStripProps {
  centers: StripCenter[];
...
export function StudentsCentersStrip({
  centers,
  studentsByCenter,
  activeCenterTabId,
  onSelectCenter,
        ...
        {hasCenterTabs ? (
          <div className="min-w-0 flex-1 overflow-x-auto [-ms-overflow-style:none] [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
            <nav className="flex min-w-max items-center gap-2.5" role="tablist" aria-label={t('centers')}>
              {centers.map((center) => {
                const count = studentsByCenter[center.id]?.length || 0;
                ...
                return (
                  <button
                    ...
                    id={`student-center-tab-${center.id}`}
              ...
              {hasUnassigned ? (
                <button
                  ...
                  id="student-center-tab-unassigned"
                ...
                >
                  ...
                  <span className="inline-flex items-center gap-1.5">
                    ...
                    >
                      {studentsByCenter.unassigned?.length || 0}
                    </span>
                  </span>
                </button>
              ) : null}
