...
import type { TeacherCalendarOrderedRow } from '@/shared/lib/calendar/teacher-calendar-list-order';
...
interface LessonListTableDesktopTableProps {
  ...
  isTeacher: boolean;
  sectionedPageRows: TeacherCalendarOrderedRow[];
  sectionedOrderedRows: TeacherCalendarOrderedRow[];
          ...
          <tbody className="divide-y divide-slate-100">
            {sectionedCalendarList
              ? sectionedPageRows.flatMap((row, idx) => {
                  const globalIdx =
                    (sectionedListPage - 1) * TEACHER_CALENDAR_LIST_PAGE_SIZE + idx;
                  const prevGlobal =
                  ...
                  const prevSection = prevGlobal
                    ? teacherCalendarRowSection(prevGlobal.category)
                    : null;
