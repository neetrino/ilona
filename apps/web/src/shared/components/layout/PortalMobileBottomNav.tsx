...
import {
  ...
  getStudentPortalHomePath,
  getTeacherPortalHomePath,
...
type PortalMobileBottomNavProps = {
  showNotifications?: boolean;
};

export function PortalMobileBottomNav({ showNotifications = false }: PortalMobileBottomNavProps) {
  ...
  const portalHomeHref = useMemo(() => {
    if (role === 'TEACHER') {
      return `/${locale}${getTeacherPortalHomePath()}`;
    }
    if (role === 'STUDENT') {
      return `/${locale}${getStudentPortalHomePath()}`;
    }
    return `/${locale}${getAdminPortalBasePath(role)}`;
  }, [locale, role]);

  const isSubpage = useMemo(() => {
    if (role === 'TEACHER') {
      return isTeacherPortalSubpage(pathWithoutLocale);
    }
    if (role === 'STUDENT') {
      return isStudentPortalSubpage(pathWithoutLocale);
    }
    return isAdminPortalSubpage(pathWithoutLocale, role);
  }, [pathWithoutLocale, role]);
  ...
  const scrollToMobileHeader = () => {
    const header = document.getElementById(PORTAL_MOBILE_HEADER_ID);
    const contentScroller = document.getElementById(PORTAL_CONTENT_SCROLL_ID);
  ...
  const navigateToTop = (callback: () => void) => {
    ...
    requestAnimationFrame(() => {
      scrollToMobileHeader();
      requestAnimationFrame(scrollToMobileHeader);
    });
  };
