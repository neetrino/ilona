...
export const TEACHER_CALENDAR_LIST_PAGE_SIZE = 10;

export type TeacherCalendarRowCategory = 'upcoming-next' | 'upcoming-later' | 'today' | 'completed';

export type TeacherCalendarOrderedRow = {
  lesson: Lesson;
  category: TeacherCalendarRowCategory;
};

export function teacherCalendarRowSection(
  category: TeacherCalendarRowCategory,
 ...
 */
export function buildTeacherCalendarOrderedRows(
  lessons: Lesson[],
  now: Date = new Date(),
): TeacherCalendarOrderedRow[] {
  const todayKey = formatScheduleDate(now);
  ...
  const rows: TeacherCalendarOrderedRow[] = [];
